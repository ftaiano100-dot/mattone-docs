...
## Scope WRITE (solo operatività ospite)
- **In scope**: `reservations/save` (modifica/estensione date), `reservations/add-payment`. Registrazione ospite: `check-in-guest`, `save-guest-pictures`.
- **Da aggiungere** (chiesto a Kross, non ancora concesso): **`add-charge`** — serve per gli upsell/servizi extra (pulizia extra, servizi in prenotazione). Va inquadrato come "gestione servizi ospite in prenotazione", **mai** come AI-messaging. Se non è nel link di condivisione dello studente, il tool non può addebitare.
