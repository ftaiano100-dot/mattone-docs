...
| **Tool AI partner: "Terna non trovata" a runtime** | Il gateway `mcp.mattone.co` **non è ancora multi-tenant** per i tool operativi (§0): risolve la terna del partner solo in validazione ("Collega Kross" → verde), non a runtime | È l'ultimo pezzo per far parlare gli agent del partner con Kross end-to-end. **In carico a Francesco** (chiusura gateway multi-tenant §0). Fino ad allora i tool AI dei partner nuovi rispondono "Terna non trovata" anche se la connessione risulta verde in admin |
