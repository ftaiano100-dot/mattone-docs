...
## Prassi Mattone: partire dal nostro snapshot GHL
Di solito **non** si parte da una location vuota: **duplichiamo il NOSTRO workspace/snapshot GHL** (un template già pronto con pipeline, automazioni, campi) nel sotto-account dello studente. Poi, in quella location, si crea il **PIT** (i passi qui sopra) e lo si collega alla piattaforma.

<Steps>
  <Step title="Duplica lo snapshot Mattone">Crea il sotto-account dello studente dallo **snapshot template** Mattone (pipeline + automazioni già impostate), invece di configurarlo da zero.</Step>
  <Step title="Crea il PIT nella sua location">Nel sotto-account appena creato → Settings → Private Integrations → crea il token con gli scope necessari (vedi sopra).</Step>
...
## Fonti ufficiali
- **GoHighLevel — Private Integrations Token** (doc ufficiale): https://marketplace.gohighlevel.com/docs/Authorization/PrivateIntegrationsToken/
- **GHL API** (riferimento): https://highlevel.stoplight.io
