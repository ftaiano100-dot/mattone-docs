...
description: "Le 7 regole d'oro che si applicano a ogni integrazione Mattone: seguirle previene l'80% dei problemi prima che si presentino."
---
...
Le chiavi API, i token e le password dei clienti non vanno mai in una chat (WhatsApp, Slack, email) né committate in un repository. Vanno sempre e solo nel **Vault** (Supabase cifrato).
...
Ogni integrazione ha un repository API di riferimento. Prima di improvvisare una soluzione o fare una supposizione su come funziona un'API, consulta il repo. Il codice è la fonte di verità — la documentazione esterna può essere obsoleta.

<Note>
  L'elenco completo dei repository con le relative integrazioni è in [Repo e API di riferimento](/riferimenti/repo-api). Aggiungilo ai segnalibri.
</Note>
